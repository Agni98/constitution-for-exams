...
| **Article page** | A breadcrumb (Home, Part, Chapter, group), previous and next, the heading and its pills. Under them, up to six tabs sort the readings into kinds: **Bare Text** (the text as printed, the articles and Schedules it refers to, the amendments that changed it, and its footnotes), **Explainer** (what it says, the catch, worth knowing), **Flow Charts** and **Mind Maps** (every drawing for the article, its section or its Part, sorted by type), **Judgments** and **For Exams**. A tab with nothing in it is left out, so the bar shows only the readings that exist. The choice is remembered from one article to the next, and the address names it, so `#/article/21/judgments` opens on the judgments. The Preamble and every Schedule use the same tabs |
