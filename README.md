...
| **Article page** | A breadcrumb (Home, Part, Chapter, group), previous and next, the heading and its pills. Under them, six tabs sort the readings into kinds: **Bare Text** (the text as printed, the articles and Schedules it refers to, the amendments that changed it, and its footnotes), **Explainer** (what it says, the catch, worth knowing), **Flow Charts** and **Mind Maps** (every drawing for the article, its section or its Part, sorted by type), **Judgments** and **For Exams**. A tab with nothing in it is disabled. The choice is remembered from one article to the next, and the address names it, so `#/article/21/judgments` opens on the judgments. The Preamble and every Schedule use the same tabs |
...
| **Landmark judgments** | 125 judgments, each on its own page at `#/judgment/<id>`, with previous and next. A judgment rewritten in full reads as: the constitutional question, the facts, what the Court held (the majority, any concurring opinion and any dissent, each in its own box), the key principles and what happened next, with the bench, the date, the citation and a link to the full text. The index is a filterable card grid, and every article's Judgments tab lists the cards filed under it |
...
The list of all 26 Parts is a drawer behind the **☰** button on every page. The Part tree folds away with the chevron in its header, and the choice is remembered.
