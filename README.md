...
|---|---|
| **Overview** | *The Constitution of India for Exams* — the document itself, in the order it is printed: the Preamble, the 26 Parts, the Schedules. Nothing else, because everything else is in the menu bar on every page. No prose, no counts row — the source note and the disclaimer sit in the footer of every page |
...
| **Part page** | The whole Part, on one page. Every section renders in the order the Constitution prints them, so opening Fundamental Rights gives you all 30 articles to read through, not the first two. The left rail is a table of contents: it lists the sections, clicking one scrolls to it, and scrolling marks where you are. Where a Part has no chapters or groups the rail lists its articles instead. `#/part/V/13` still opens at the Union Judiciary — it scrolls there rather than hiding the rest |
| **Reading an article** | Every article is read **in the same shell**. The rail stays on the left with its section open and every article in it listed, and the mind map is a strip under the breadcrumb that stays open, once opened, for every article in that section. Previous and next run through the **whole document**, not one Part — article 11 leads to article 12, and the foot pager names the Part it is putting you in. The only two ends are the Preamble, before article 1, and the Schedules, after article 395 |
